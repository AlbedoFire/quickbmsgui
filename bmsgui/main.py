...
def view_list():
    cmd = ["D:\\source\\unpacktool\\quickbms_win\\quickbms_4gb_files.exe", "-l",bms_script_path.get(),file_path.get()
           ,out_path.get()]
    threading.Thread(target=run_long_process,args=(cmd,), daemon=True).start()
def unpack_d():
    cmd = ["D:\\source\\unpacktool\\quickbms_win\\quickbms_4gb_files.exe", bms_script_path.get(), file_path.get()
        , out_path.get()]
    threading.Thread(target=run_long_process, args=(cmd,), daemon=True).start()
# 创建主窗口
root = tk.Tk()
